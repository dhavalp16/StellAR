...
using TMPro; // Added for dynamic buttons
using UnityEngine.XR.Interaction.Toolkit.Samples.ARStarterAssets; // Added for DynamicModelLoader
...
namespace UnityEngine.XR.Templates.AR
{
    ...
    public class ARTemplateMenuManager : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField]
        [Tooltip("The container for object buttons.")]
        Transform m_MenuContent;

        [SerializeField]
        [Tooltip("Prefab for the dynamic object placeholder.")]
        GameObject m_DynamicPlaceholderPrefab;

        void Start()
        {
            // --- Auto-Assign Missing References (Robustness) ---
            if (m_ObjectMenuAnimator == null) 
            {
                m_ObjectMenuAnimator = GetComponentInParent<Animator>();
                if (m_ObjectMenuAnimator == null) m_ObjectMenuAnimator = GetComponent<Animator>();
            }

            if (m_CreateButton == null)
            {
                var go = GameObject.Find("Create Button");
                if (go) m_CreateButton = go.GetComponent<Button>();
            }

            if (m_DeleteButton == null)
            {
                var go = GameObject.Find("Delete Button");
                if (go) m_DeleteButton = go.GetComponent<Button>();
            }
            
            if (m_CancelButton == null)
            {
                 // Cancel button is usually a child of the menu
                 var go = GetComponentInChildren<Button>(true); 
                 // But wait, the script is on Viewport, Cancel Button is sibling of Scroll View usually.
                 // Searching broadly in parent "Object Menu"
                 if (transform.parent != null && transform.parent.parent != null)
                 {
                     var menu = transform.parent.parent; // ScrollView -> ObjectMenu
                     var cancelChild = menu.Find("Cancel Button");
                     if (cancelChild) m_CancelButton = cancelChild.GetComponent<Button>();
                 }
            }
            ...
            // Smart Startup: 
            // If the script is starting right at the beginning of the scene, hide the menu.
            // If it's starting later (e.g. user clicked a button to activate the GameObject), show it.
            ...
            {
                // We were likely activated by a user action, so show the menu!
                ShowMenu();
            }
        }
        ...
        {
            if (m_MenuContent == null || m_MenuContent.childCount == 0) return;
            
            // Hook up selection logic for EXISTING (static) buttons
            foreach (Transform child in m_MenuContent)
            {
                var btn = child.GetComponent<Button>();
                if (btn != null)
                {
                    var childGo = child.gameObject;
                    btn.onClick.AddListener(() => UpdateSelectionVisuals(childGo));
                }
            }
            
            // Use first button as template
            var templateButton = m_MenuContent.GetChild(0).gameObject;
            
            string modelsPath = System.IO.Path.Combine(Application.persistentDataPath, "models");
            Debug.Log($"[DynamicMenu] Scanning for models at: {modelsPath}");
            
            if (!System.IO.Directory.Exists(modelsPath)) 
            {
                Debug.LogWarning($"[DynamicMenu] Directory not found: {modelsPath}");
                return;
            }

            var files = System.IO.Directory.GetFiles(modelsPath, "*.glb");
            Debug.Log($"[DynamicMenu] Found {files.Length} models.");
            foreach (var file in files)
            {
                var btnGo = Instantiate(templateButton, m_MenuContent);
                var btn = btnGo.GetComponent<Button>();
                
                // RESET SELECTION STATE (Important!)
                // The template might be selected, so the clone will have SelectionBox active.
                // We must turn it off.
                var selectionBox = btnGo.transform.Find("SelectionBox");
                if (selectionBox != null) selectionBox.gameObject.SetActive(false);

                // Try TMP then Text
                var tmp = btnGo.GetComponentInChildren<TMPro.TMP_Text>();
                if (tmp != null) tmp.text = System.IO.Path.GetFileNameWithoutExtension(file);
                else 
                {
                    var txt = btnGo.GetComponentInChildren<Text>();
                    if (txt != null) txt.text = System.IO.Path.GetFileNameWithoutExtension(file);
                }

                // Ensure RarityManager exists
                if (RarityManager.Instance == null)
                {
                    var rarityGo = new GameObject("RarityManager");
                    rarityGo.AddComponent<RarityManager>();
                }

                // Check/Create Metadata (DEBUG: Creates random rarity if missing)
                RarityManager.Instance.GenerateDebugMetadata(file);

                // Apply Rarity Color
                string rarity = RarityManager.Instance.GetRarityForFile(file);
                Color rarityColor = RarityManager.Instance.GetColor(rarity);

                // --- BORDER ONLY FIX ---
                // Instead of coloring the background, we clone the SelectionBox (which is a border)
                // and use it as a permanent "Rarity Frame".
                // var selectionBox = btnGo.transform.Find("SelectionBox"); // Already found above
                if (selectionBox != null)
                {
                    // Clone the SelectionBox to make a RarityFrame
                    var rarityFrame = Instantiate(selectionBox.gameObject, btnGo.transform);
                    rarityFrame.name = "RarityFrame";
                    
                    // Setup RarityFrame
                    rarityFrame.SetActive(true); // Always visible
                    var frameImg = rarityFrame.GetComponent<Image>();
                    if (frameImg != null)
                    {
                        frameImg.color = rarityColor;
                    }
                    
                    // Ensure it stays behind the real SelectionBox (if that becomes active)
                    rarityFrame.transform.SetSiblingIndex(selectionBox.GetSiblingIndex());
                }
                else
                {
                    // Fallback to background tint if no border found (safeguard)
                    var btnImg = btnGo.GetComponent<Image>();
                    if (btnImg != null) btnImg.color = rarityColor;
                }
                // -----------------------

                btn.onClick.RemoveAllListeners();
                var localPath = file; // Capture for lambda
                btn.onClick.AddListener(() => {
                    SetDynamicObject(localPath);
                    UpdateSelectionVisuals(btnGo);
                });

                // Try auto-generate thumbnail
                var iconImage = btnGo.transform.Find("ObjectIcon")?.GetComponent<Image>();
                if (iconImage != null)
                {
                    // Ensure Generator exists
                    if (ThumbnailGenerator.Instance == null)
                    {
                         var genGo = new GameObject("ThumbnailGenerator");
                         genGo.AddComponent<ThumbnailGenerator>();
                    }
                    
                    // DEBUG: Force REGENERATE every time for now (Delete mismatched caches)
                    // string thumbPath = System.IO.Path.ChangeExtension(file, ".png");
                    // if (System.IO.File.Exists(thumbPath)) System.IO.File.Delete(thumbPath);

                    ThumbnailGenerator.Instance.GenerateThumbnailFor(file, iconImage);
                }
            }
        }

        void UpdateSelectionVisuals(GameObject selectedButton)
        {
            if (m_MenuContent == null) return;
            foreach (Transform child in m_MenuContent)
            {
                var selectionBox = child.Find("SelectionBox");
                if (selectionBox != null)
                {
                    bool isSelected = (child.gameObject == selectedButton);
                    selectionBox.gameObject.SetActive(isSelected);
                }
            }
        }

        void SetDynamicObject(string path)
        {
             if (DynamicModelLoader.Instance == null)
             {
                 Debug.LogError("CRITICAL: DynamicModelLoader Instance is null. You MUST create an empty GameObject in the scene and attach the 'Dynamic Model Loader' script to it.");
                 return;
             }
             
             DynamicModelLoader.Instance.PendingPath = path;
             
             if (m_DynamicPlaceholderPrefab != null && m_ObjectSpawner != null)
             {
                 // Ensure placeholder is in list
                 if (!m_ObjectSpawner.objectPrefabs.Contains(m_DynamicPlaceholderPrefab))
                 {
                     m_ObjectSpawner.objectPrefabs.Add(m_DynamicPlaceholderPrefab);
                 }
                 
                 int index = m_ObjectSpawner.objectPrefabs.IndexOf(m_DynamicPlaceholderPrefab);
                 SetObjectToSpawn(index);
             }
             else
             {
                 Debug.LogWarning("Dynamic Placeholder Prefab not assigned in ARTemplateMenuManager!");
             }
        }
        ...
        void Update()
        {
            ...
            if (m_ShowObjectMenu || m_ShowOptionsModal)
            {
                ...
                {
                    if(m_DeleteButton != null) m_DeleteButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
                }
        ...
        {
            if (m_ObjectMenuAnimator != null) m_ObjectMenuAnimator.SetBool("Show", false);
            m_ShowObjectMenu = false;
            AdjustARDebugMenuPosition();
        }
